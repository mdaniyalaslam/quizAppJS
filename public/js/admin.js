...
function codeSubmit(){
    console.log('works', codeInput.value)
    if(codeInput.value === "QW4HD"){
        location = "admin.html"
...
function fetchDatabase(){
    database.child("quizData").child('allQuizzes').once("value", (snap) => {
        ...
        for (var i in allQuizes) {
            ...
            dltBtn.setAttribute("class", "btn btn-danger strt-btn")
            var dltBtnTxt = document.createTextNode("Delete")
            dltBtn.appendChild(dltBtnTxt)
            quizName.appendChild(dltBtn);
...
function dlt(w){
    ...
    console.log(text);
    database.child("quizData").child("allQuizzes").child(text).on("value", (snap)=>{
        var fbObj = snap.val()
        var tId = fbObj.tId
        console.log(fbObj.tId)
        database.child("quizData").child(tId).child(text).remove()
        database.child("quizData").child("allQuizzes").child(text).remove()
        location.reload()
